...
class StdOutReporter(BaseReporter):
    ...
    def end_generation(
        ...
        if self.show_species_detail:
            ...
            sids.sort()
            print(
                "   ID   age  size       fitness        best_fitness        ref_point  max_ref_point    adj fit     temperature  stag"
            )
            print(
                "  ====  ===  ====  ===============  ===============  =============    =============     =============== ======= ===="
            )
            ...
            for sid in sids:
                ...
                if s.best_fitness:
                    bf = [
                        round(s.best_fitness[i], 4) for i in range(len(s.best_fitness))
                    ]
                    bf = "{0}".format(bf)
                else:
                    bf = "--"
                af = (
                    "--"
                    if s.adjusted_fitness is None
                    else "{:.3f}".format(s.adjusted_fitness)
                )
                """
                ...
                if s.ref_points is not None:
                    r_pts = [
                        round(s.ref_points[i], 4) for i in range(len(s.ref_points))
                    ]
                ...
                pf = (
                    "--"
                    if s.priority_fitness is None
                    else "{:.4f}".format(s.priority_fitness)
                )
                if s.best_ref_points is not None:
                    mxr_pts = [
                        round(s.best_ref_points[i], 4)
                        for i in range(len(s.best_ref_points))
                    ]
                ...
                print(
                    "  {: >4}  {: >3}  {: >4}  {: >4} {: >4}  {: >4}  {: >4} {: >4} {: >4}  {: >4}".format(
                        sid, a, n, f, bf, r_pts, mxr_pts, af, tempe, st
                    )
                )
                self.filewrite(
                    "  {: >4}  {: >3}  {: >4}  {: >4} {: >4} {: >4} {: >4} {: >4}  {: >4}  {: >4}".format(
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        if type(fitnesses[0]) is float:
            ...
            fit_std = stdev(fitnesses)
            print(
                "Population's average fitness: {0:3.3f} stdev: {1:3.3f}".format(
                    fit_mean, fit_std
                )
            )
            self.filewrite(
            ...
            print(
                "Population's average fitness: {0} stdev: {1}".format(fit_mean, fit_std)
            )
            self.filewrite(
                "Population's average fitness: {0} stdev: {1}".format(fit_mean, fit_std)
            )
        ...
        print(
            "Best fitness: {0} - size: {1!r} - species {2} - id {3}".format(
                best_genome.fitness,
                best_genome.size(),
                best_species_id,
                best_genome.key,
    ...
    def genetic_distance(self, config, distance):
        ...
        if (
            ...
            and (self.generation + 1) % config.output_interval == 0
        ):
            visualize.plot_stats2D(
                ...
                ylog=False,
                view=False,
                ylabel="Genome Distance mean",
                filename="{}/GenomeDistance.png".format(self.outpath),
                title="Genome Distance mean",
            )
