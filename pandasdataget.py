...
def plot_stats2D(
    ...
    if threshold != 0:
        threshold_y = np.full(len(plot_x), threshold)
        plt.plot(plot_x, threshold_y, "y-", label="threshold")
    plt.title(title)
...
def main():
    ...
    if dataname == "hv_history":
        plot_stats2D(
            ...
            ylabel="Hypervolume",
            filename=str(len(all_data.T))
            + "_trials_"
            + str(method)
            + "_HyperVolume_new.png",
            title=str(method)
            + " "
            + str(len(all_data.T))
            + " trials Hypervolume describe",
            ymin=0,
            ymax=1.333,
            threshold=0,
        )
        ...
        plot_stats2D(
            ...
            + " "
            + str(len(all_data.T))
            + " trials Genetic distance describe",
            ymin=1.25,
            ymax=3.5,
            threshold=2.85,
        )
