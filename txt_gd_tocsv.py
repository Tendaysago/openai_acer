...
def main():
    ...
    while len(log_historys)>0 and datanum>0:
        ...
        with open(log) as f:
            ...
            for s_line in f:
                ...
                if "members" in s_line and "Population" in s_line and "species" in s_line:
                    matchpattern = re.sub("\D","",s_line)
                    #print(matchpattern)
                    populationnum, speciesnum = int(matchpattern[0:3]),int(matchpattern[3:])
        ...
        if(generationnum==generation):
            ...
            speciesnumdata.to_csv(str(method)+"speciesnum_history"+ str(int(args.usenum)-datanum) +".csv")
            speciesnum_historys.append(str(method)+"speciesnum_history"+ str(int(args.usenum)-datanum) +".csv")
            populationnumdata.to_csv(str(method)+"populationnum_history" + str(int(args.usenum)-datanum) + ".csv")
            populationsnum_historys.append(str(method)+"populationnum_history" + str(int(args.usenum)-datanum) + ".csv")

    while(len(speciesnum_historys)>0):
        if column==-1:
            column=0
            all_speciesnum_data=pd.read_csv(speciesnum_historys.pop(0),index_col=0)
            all_populationsnum_data=pd.read_csv(populationsnum_historys.pop(0),index_col=0)
        else:
            column+=1
            next_speciesnum_data = pd.read_csv(speciesnum_historys.pop(0),index_col=0)
            next_speciesnum_data = next_speciesnum_data.rename(columns={'0':str(column)})
            next_populationsnum_data = pd.read_csv(populationsnum_historys.pop(0),index_col=0)
            next_populationsnum_data = next_populationsnum_data.rename(columns={'0':str(column)})
            all_speciesnum_data = pd.concat([all_speciesnum_data, next_speciesnum_data], axis=1)
            all_populationsnum_data = pd.concat([all_populationsnum_data, next_populationsnum_data], axis=1)
    ...
    print(quartile_100)
    """
    plot_stats2D(generation, means=species_means, stds=species_stds, mins=species_quartile_0, quartile_25=species_quartile_25, quartile_50=species_quartile_50, quartile_75=species_quartile_75, maxs=species_quartile_100, ylog=False, view=False, ylabel='Species num',filename=str(len(all_speciesnum_data.T))+'_trials_'+str(method)+'_speciesnum.png',title=str(method) + ' ' + str(len(all_speciesnum_data.T)) +' trials Species num describe', ymin=0, ymax=10)
    plot_stats2D(generation, means=populations_means, stds=populations_stds, mins=populations_quartile_0, quartile_25=populations_quartile_25, quartile_50=populations_quartile_50, quartile_75=populations_quartile_75, maxs=populations_quartile_100, ylog=False, view=False, ylabel='Populations num',filename=str(len(all_populationsnum_data.T))+'_trials_'+str(method)+'_populationsnum.png',title=str(method) + ' ' + str(len(all_populationsnum_data.T)) +' trials Populations num describe', ymin=190,ymax=400)
    #print(all_data)
